...
module.exports = {
    getAnimeLinksList: async (name) => {
        ...
        // 2 - Find the input (class="search-box") and type "vostfree"
        await page.type('input.search-box', name)

        // 3 - Click the "Search" button (class="search-button")
        ...
        // 4 - Get all results (div class="search-results")
        const links = await page.$$eval('.search-result .title a', links => links.map((link, index) => {
        ...
        for (const link of links) {
            const probableSeasonFounded = link.title.split(' ').map(word => {
                if (parseInt(word) >= 0) {
                    return word;
                }
            }).join('');
            ...
            link.title = link.title.split(' ').map(word => {
                if (word === 'french' || word === 'vost' || word === 'vostfr' || word === 'vf') {
                    return '';
                } else {
                    return word
                }
